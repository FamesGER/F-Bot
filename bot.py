...
async def dailyDuck():	
	...
	while not bot.is_closed:
		...
		if timeNow.hour == 11 and timeNow.minute == 1: #DE 13:01
...
async def on_message(message):
	...
	if message.content.upper().startswith('YEE HAW') or message.content.upper().startswith('YEEHAW') or message.content.upper().startswith('YEEYEE') or message.content.upper().startswith('YEE YEE') or message.content.upper().startswith('YIPPY KAI YAY') or message.content.upper().startswith('🤠'):
		try:
			await bot.add_reaction(message,emoji="🤠")
		except:
			print("sad")
